...
export default function EmployeeDetailsPage() {
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center py-24 text-slate-455 gap-3" dir={isAr ? 'rtl' : 'ltr'}>
        ...
        <p className="text-sm font-medium text-slate-400">{isAr ? 'جاري تحميل تفاصيل الموظف...' : 'Loading employee details...'}</p>
      </div>
    );
  }
  ...
  if (error || !employee) {
    return (
      <div className="flex flex-col items-center py-20 text-slate-400 gap-4" dir={isAr ? 'rtl' : 'ltr'}>
    ...
    <div className="space-y-8 animate-in fade-in duration-300" dir={isAr ? 'rtl' : 'ltr'}>
      ...
      <div className={`flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-slate-900/40 p-6 rounded-2xl border border-slate-800 backdrop-blur-xl`}>
        <div className="flex items-center gap-4">
          <Link
            ...
            className="p-3 bg-slate-800/80 hover:bg-slate-700/80 border border-slate-700/50 hover:border-slate-650 rounded-xl text-slate-355 hover:text-white transition-all cursor-pointer"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="space-y-6 lg:col-span-1">
          <Card className="border-slate-800 bg-slate-900/30 overflow-hidden backdrop-blur-xl">
            <CardContent className="pt-8 flex flex-col items-center text-center">
              ...
              {/* Role Display */}
              <div className="inline-flex items-center gap-1.5 bg-slate-950 border border-slate-850 px-3 py-1.5 rounded-xl text-xs text-slate-300 font-mono mb-6">
                <Shield className="w-3.5 h-3.5 text-indigo-455" />
                <span>{isAr ? 'صلاحية النظام: ' : 'System Role: '}</span>
                <strong className="text-white font-bold">{getLocalizedRole(employee.role)}</strong>
              </div>

              {/* Status Notice for non-admins */}
              {!isCompanyAdmin && (
                <div className={`w-full bg-slate-950/40 border border-slate-850 p-4 rounded-xl text-xs text-slate-400 leading-relaxed ${isAr ? 'text-right' : 'text-left'}`}>
          ...
          <Card className="border-slate-800 bg-slate-900/20 backdrop-blur-xl">
            <CardContent className={`p-6 space-y-4 ${isAr ? 'text-right' : 'text-left'}`}>
              <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2 border-b border-slate-800 pb-2">
              ...
              <div className="flex items-center gap-3 text-xs">
                ...
                <div className="min-w-0">
                  <p className="text-slate-400">{isAr ? 'البريد الإلكتروني' : 'Email Address'}</p>
              ...
              <div className="flex items-center gap-3 text-xs">
                ...
                <div>
                  <p className="text-slate-400">{isAr ? 'حالة الحساب' : 'Account Status'}</p>
              ...
              <div className="flex items-center gap-3 text-xs">
                ...
                <div>
                  <p className="text-slate-400">{isAr ? 'تاريخ التسجيل بالنظام' : 'Registered Date'}</p>
        ...
        <div className="lg:col-span-2">
          <Card className="border-slate-800 bg-slate-900/30 backdrop-blur-xl">
            <CardContent className="p-6">
              ...
              <h3 className={`text-base font-bold text-white mb-6 flex items-center gap-2 border-b border-slate-800 pb-3 ${isAr ? 'text-right' : 'text-left'}`}>
              ...
              <div className={`space-y-6 ${isAr ? 'text-right' : 'text-left'}`}>
                ...
                <div className="space-y-4">
                  <h4 className="text-xs font-bold text-slate-400 border-b border-slate-850 pb-1.5 uppercase tracking-wide">
                    {isAr ? 'المعلومات الشخصية والاتصال' : 'Personal Information & Contact'}
                  </h4>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                      <p className="text-xs text-slate-500">{isAr ? 'الاسم الأول' : 'First Name'}</p>
                      <p className="text-sm font-semibold text-slate-200 mt-1">{employee.profile?.first_name || '-'}</p>
                    </div>
                    <div>
                      <p className="text-xs text-slate-500">{isAr ? 'اسم العائلة' : 'Last Name'}</p>
                      <p className="text-sm font-semibold text-slate-200 mt-1">{employee.profile?.last_name || '-'}</p>
                    </div>
                    <div>
                      <p className="text-xs text-slate-500">{isAr ? 'البريد الإلكتروني' : 'Email Address'}</p>
                      <p className="text-sm font-semibold text-slate-200 font-mono mt-1">{employee.email}</p>
                    </div>
                    <div>
                      <p className="text-xs text-slate-500">{isAr ? 'رقم الجوال' : 'Phone Number'}</p>
                ...
                <div className="space-y-4 border-t border-slate-850/60 pt-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                      <p className="text-xs text-slate-500">{isAr ? 'المسمى الوظيفي' : 'Job Title'}</p>
                      <p className="text-sm font-semibold text-slate-200 mt-1">{getLocalizedRole(employee.role)}</p>
                    </div>
                    <div>
                      <p className="text-xs text-slate-500">{isAr ? 'التخصص / القسم' : 'Specialization / Department'}</p>
                      <p className="text-sm font-semibold text-slate-200 mt-1">{employee.profile?.specialization || '-'}</p>
                    </div>
                    <div>
                      <p className="text-xs text-slate-500">{isAr ? 'الراتب الأساسي الشهري' : 'Basic Monthly Salary'}</p>
                    ...
                    <div>
                      <p className="text-xs text-slate-500">{isAr ? 'تاريخ المباشرة / التعيين' : 'Hiring / Joining Date'}</p>
                ...
                <div className="space-y-4 border-t border-slate-850/60 pt-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                      <p className="text-xs text-slate-500">{isAr ? 'رقم الهوية الوطنية / الإقامة' : 'National ID / Iqama Number'}</p>
                      <p className="text-sm font-semibold text-slate-200 font-mono mt-1">{employee.profile?.national_id || '-'}</p>
                    </div>
                    <div>
                      <p className="text-xs text-slate-500">{isAr ? 'تاريخ انتهاء الهوية' : 'ID Expiry Date'}</p>
                      <p className="text-sm font-semibold text-slate-200 mt-1">
              ...
              <div className="grid grid-cols-1 gap-4">
                <div className="space-y-1.5">
                  ...
                  <input
                    ...
                    className="w-full bg-slate-950 border border-slate-800 focus:border-indigo-500 disabled:opacity-50 text-slate-200 text-sm rounded-xl p-3 outline-none transition-all font-mono"
