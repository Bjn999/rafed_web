...
export default function DashboardPage() {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      default:
        return (
          <span className="bg-slate-500/10 text-muted-foreground border border-slate-500/20 text-xs px-2.5 py-1 rounded-full font-semibold">
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-300">
      ...
      {/* Welcome Section */}
      <div className="bg-gradient-to-r from-primary/10 to-purple-500/10 border border-primary/20 rounded-2xl p-6 sm:p-8 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6 relative overflow-hidden backdrop-blur-xl">
        <div className="space-y-2">
          <h1 className={`text-2xl sm:text-3xl font-bold text-foreground ${isAr ? 'text-right' : 'text-left'}`}>
            {isAr ? 'أهلاً بك يا،' : 'Welcome,'} {profile?.first_name} {profile?.last_name} 👋
          </h1>
          <p className={`text-muted-foreground text-sm max-w-2xl ${isAr ? 'text-right' : 'text-left'}`}>
            {isAr 
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
        <Card className="border-border bg-card backdrop-blur text-foreground hover:border-border transition-all">
          <CardHeader className="flex flex-row items-center justify-between pb-2 space-y-0">
            <CardTitle className="text-xs sm:text-sm font-medium text-muted-foreground">{isAr ? 'إجمالي المشاريع' : 'Total Projects'}</CardTitle>
        ...
        <Card className="border-border bg-card backdrop-blur text-foreground hover:border-border transition-all">
          <CardHeader className="flex flex-row items-center justify-between pb-2 space-y-0">
            <CardTitle className="text-xs sm:text-sm font-medium text-muted-foreground">{isAr ? 'أعضاء الفريق' : 'Team Members'}</CardTitle>
        ...
        <CardContent className="p-0">
          {projectsLoading ? (
            ...
            <div className="flex flex-col items-center py-12 text-muted-foreground gap-3">
              <div className="w-12 h-12 bg-muted rounded-2xl border border-border flex items-center justify-center text-muted-foreground">
                <FolderKanban className="w-6 h-6" />
              </div>
              <p className="text-sm font-semibold">{isAr ? 'لا توجد مشاريع مضافة لمؤسستكم حالياً' : 'No projects have been added yet'}</p>
              <p className="text-xs text-muted-foreground">{isAr ? 'انقر على "إنشاء مشروع جديد" بالأعلى للبدء' : 'Click "Create New Project" above to get started'}</p>
            </div>
          ) : (
            <div className="overflow-x-auto">
              <table className={`w-full ${isAr ? 'text-right' : 'text-left'} border-collapse text-sm`}>
                <thead>
                  <tr className="border-b border-border bg-background/20 text-muted-foreground text-xs font-semibold">
