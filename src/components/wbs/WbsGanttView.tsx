...
export default function WbsGanttView({
  ...
  const getTaskLayout = (item: WbsItemData) => {
    ...
    if (isAr) {
      // RTL: Timeline 0% is at RIGHT (Project Start Date)
      const rightVal = Math.min(98, Math.max(0, startPercent));
      return {
        style: {
          right: `${rightVal}%`,
          width: `${Math.min(100 - rightVal, widthPercent)}%`,
        },
      };
    } else {
      // LTR: Timeline 0% is at LEFT (Project Start Date)
      const leftVal = Math.min(98, Math.max(0, startPercent));
      return {
        style: {
          left: `${leftVal}%`,
          width: `${Math.min(100 - leftVal, widthPercent)}%`,
        },
      };
    }
  };
          ...
          <div className="flex items-center gap-1 bg-slate-950 border border-slate-800 p-1 rounded-xl text-[10px]">
            ...
            <button
              ...
              className={`px-2 py-1 rounded-lg font-bold transition-all cursor-pointer ${
                zoomMode === 'auto' ? 'bg-indigo-600 text-white' : 'text-slate-400 hover:text-white'
              }`}
            >
              {isAr ? 'تلقائي' : 'Auto'}
            </button>
            <button
              ...
              className={`px-2 py-1 rounded-lg font-bold transition-all cursor-pointer ${
                zoomMode === '5days' ? 'bg-indigo-600 text-white' : 'text-slate-400 hover:text-white'
              }`}
            >
            ...
            <button
              ...
              className={`px-2 py-1 rounded-lg font-bold transition-all cursor-pointer ${
                zoomMode === '10days' ? 'bg-indigo-600 text-white' : 'text-slate-400 hover:text-white'
              }`}
            >
            ...
            <button
              ...
              className={`px-2 py-1 rounded-lg font-bold transition-all cursor-pointer ${
                zoomMode === 'monthly' ? 'bg-indigo-600 text-white' : 'text-slate-400 hover:text-white'
              }`}
            >
              {isAr ? 'شهري' : 'Monthly'}
            </button>
