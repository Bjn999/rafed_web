...
export default function WbsTab({ projectId, projectStartDate, projectEndDate }: WbsTabProps) {
  ...
  if (isError) {
    return (
      <div className="flex flex-col items-center py-12 text-muted-foreground gap-3">
        ...
        <button
          onClick={() => refetch()}
          className="px-4 py-2 rounded-xl bg-muted hover:bg-slate-700 text-foreground text-xs font-semibold flex items-center gap-2 cursor-pointer"
        >
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300" dir={isAr ? 'rtl' : 'ltr'}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Card className="border-border bg-card text-foreground">
          <CardContent className="pt-5 flex items-start gap-4">
            ...
            <div className="space-y-1 min-w-0 flex-1">
              <span className="text-[10px] text-muted-foreground block font-bold">{isAr ? 'نسبة الإنجاز الإجمالية' : 'Overall Completion'}</span>
              ...
              <div className="w-full bg-background rounded-full h-1.5 overflow-hidden border border-border mt-1">
        ...
        <Card className="border-border bg-card text-foreground">
          <CardContent className="pt-5 flex items-start gap-4">
            ...
            <div className="space-y-0.5">
              <span className="text-[10px] text-muted-foreground block font-bold">{isAr ? 'إجمالي التكلفة المقدرة' : 'Total Estimated Budget'}</span>
              <span className="text-base font-bold font-mono text-foreground block">{formatCurrency(summary.total_estimated_cost)}</span>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="border-border bg-card text-foreground">
          <CardContent className="pt-5 flex items-start gap-4">
            ...
            <div className="space-y-0.5">
              <span className="text-[10px] text-muted-foreground block font-bold">{isAr ? 'إجمالي المنصرف الفعلي' : 'Total Actual Cost'}</span>
        ...
        <Card className="border-border bg-card text-foreground">
          <CardContent className="pt-5 flex items-start gap-4">
            ...
            <div className="space-y-0.5">
              <span className="text-[10px] text-muted-foreground block font-bold">{isAr ? 'حالة الأنشطة والتنفيذ' : 'Activities Status'}</span>
      ...
      {/* Toolbar & Filters */}
      <div className="flex flex-col md:flex-row items-stretch md:items-center justify-between gap-4 bg-card p-4 border border-border rounded-2xl">
        ...
        <div className="flex flex-wrap items-center gap-3 flex-1">
          ...
          <div className="relative flex-1 min-w-[200px]">
            <Search className={`w-4 h-4 text-muted-foreground absolute top-1/2 -translate-y-1/2 ${isAr ? 'right-3' : 'left-3'}`} />
            <input
              type="text"
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              placeholder={isAr ? 'البحث بالاسم أو الرمز...' : 'Search by name or code...'}
              className={`w-full bg-background border border-border rounded-xl text-xs text-foreground py-2 focus:outline-none focus:border-indigo-500 transition-colors ${
          ...
          <select
            ...
            className="bg-background border border-border text-xs text-foreground rounded-xl px-3 py-2 focus:outline-none focus:border-indigo-500 cursor-pointer"
          >
        ...
        <div className="flex items-center gap-2 shrink-0">
          <Link
            ...
            className="bg-muted hover:bg-slate-700 text-indigo-400 hover:text-indigo-300 border border-border rounded-xl py-2.5 px-4 text-xs font-bold flex items-center justify-center gap-2 transition-all cursor-pointer"
          >
